...
public class CheckForUpdates {
  ...
  public static boolean compareVersionFile() {
    ...
    String appversionfile =
        OSystem.getCurrentDir() + OSystem.getSystemSeparator() + RENCODERCURRENTVERSIONFILE;
    ...
    try {
      if (!appversion.exists())
        Logger.error("Local version file is missing, update check was aborted");
      ...
      if (serverurl == null && serverurl.isEmpty())
        Logger.error("Url of the server for updates is not defined, thus update check was aborted");
      ...
      if (versionfilename == null && versionfilename.isEmpty())
        Logger.error("Version file on server is missing, thus update check was aborted");


      String checkversionfile =
          OSystem.getCurrentDir() + OSystem.getSystemSeparator() + RENCODERUPDATECHECKVERSIONFILE;
      ...
      if (serverurl != null && versionfilename != null) {
        ...
        if (conn.getContentLength()==-1) {
          Logger.error("Cannot connect to server in order to check updates.");
          return false;
        }
  ...
  public static String extractversion(String line) {
    Pattern pat =
        Pattern.compile("(\\s+)*version:(\\d+.\\d+.\\d+)(\\s+)*", Pattern.CASE_INSENSITIVE);
