...
public class VideoAV1EncodingInfoContainer extends DefaultEncodingInfoContainer {
	...
	private PixelFormatAV1 pixformat = PixelFormatAV1.yuv420p;
	private int cpuused = 1; // Quality/Speed ratio modifier (from 0 to 8) (default 1)
	private int laginframes = -1; // Number of frames to look ahead at for alternate reference frame selection
									// (from -1 to INT_MAX) (default -1)
	private int staticthresh = 0; // A change threshold on blocks below which they will be skipped by the encoder
	...
	public void setDefaultInfo() {
		this.videocodec = VideoCodecs.AV1;
		...
		this.videoVBRQuality = new IndexedHashMap<String, Integer>() {
			...
			{
				put(VideoCodecs.AV1.toString(), 30);
			}
		};

	}

	public PixelFormatAV1 getPixformat() {
		return pixformat;
	}

	public void setPixformat(PixelFormatAV1 pixformat) {
	...
	protected ArrayList<VideoContainers> getSupportedFormats() {
		return Arrays.stream(VideoCodecs.AV1.supportsOutputFormats()).collect(Collectors.toCollection(ArrayList::new));
	}
	...
	public void setConfigurations(PropertiesConfiguration props) {
		...
		setPixformat(PixelFormatAV1.getPixelFormatFromString(
		...
		setLaginframes(PropertiesWorker.checkProperty(props, StaticVideoEncoderFields.AV1LAGINFRAMES, -1,
				Integer.MAX_VALUE, -1));
		setStaticthresh(PropertiesWorker.checkProperty(props, StaticVideoEncoderFields.AV1STATICTRESH, 0,
				Integer.MAX_VALUE, 0));
		setNoisesensitivity(PropertiesWorker.checkProperty(props, StaticVideoEncoderFields.AV1NOISESENS, 0, 4, 0));
		setTiles(PropertiesWorker.checkProperty(props, StaticVideoEncoderFields.AV1TILES, 0, Integer.MAX_VALUE, 0));
		setTilecolumns(PropertiesWorker.checkProperty(props, StaticVideoEncoderFields.AV1TILECOLUMNS, -1, 6, -1));
		setTilerows(PropertiesWorker.checkProperty(props, StaticVideoEncoderFields.AV1TILEROWS, -1, 6, -1));
	...
	public ArrayList<String> getSpecificVideoEncoderParameters(ArrayList<String> appendcmds) {
		...
		if (!getPixformat().equals(PixelFormatAV1.yuv420p)) {
