...
public class H264ExtraInfoContainer implements IExtraInfoContainer {
  ...
  public H264ExtraInfoContainer(int keyint, int minkeyint, int scenecut, int bframes, int badapt,
      boolean bpyramid, boolean nocabac, int ref, boolean nodeblock, int deblockalpha,
      int deblockbeta, int maxrate, int minrate, int bufsize, int qmin, int qmax, int qdiff,
      String partitions, int directpred, String memethod, int subq, int trellis) {
    super();
    this.keyint = keyint;
  ...
  public IExtraInfoContainer clone() {
    return new H264ExtraInfoContainer(keyint, minkeyint, scenecut, bframes, badapt, bpyramid,
        nocabac, ref, nodeblock, deblockalpha, deblockbeta, maxrate, minrate, bufsize, qmin, qmax,
        qdiff, partitions, directpred, memethod, subq, trellis);
  }
  ...
  public static H264ExtraInfoContainer loadH264ExtraInfoContainer(PropertiesConfiguration props) {
    ...
    if (props.containsKey(KEYINT) || props.containsKey(MINKEYINT) || props.containsKey(SCENECUT)
        || props.containsKey(BFRAMES) || props.containsKey(BADAPT) || props.containsKey(BPYRAMID)
        || props.containsKey(NOCABAC) || props.containsKey(REF) || props.containsKey(NODEBLOCK)
        || props.containsKey(DEBLOCKALPHA) || props.containsKey(DEBLOCKBETA)
        || props.containsKey(BUFSIZE) || props.containsKey(MINRATE) || props.containsKey(MAXRATE)
        || props.containsKey(QDIFF) || props.containsKey(QMIN) || props.containsKey(QMAX)
        || props.containsKey(PARTITIONS) || props.containsKey(DIRECTPRED)
        || props.containsKey(MEMETHOD) || props.containsKey(SUBQ) || props.containsKey(TRELLIS)) {
