...
public class EncodingPropsAuxiliar {
	...
	public static int getMaxMuxingQueueSize() {
		...
		if (prop.containsKey(StaticGlobalFields.MAXMUXINGQUEUESIZE)) {
			if (!prop.getProperty(StaticGlobalFields.MAXMUXINGQUEUESIZE).toString().isEmpty()) {
				String nproc = (String) prop.getProperty(StaticGlobalFields.MAXMUXINGQUEUESIZE);
				numberproc = Integer.valueOf(nproc);
			}
		}
		return numberproc;
